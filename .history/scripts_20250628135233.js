...
// Elimina la declaración duplicada de 'translations' si este bloque se inserta varias veces
if (typeof window.translations === "undefined") {
  window.translations = {
    en: {
      ...
      heroSubtitle: "Mathematics & Computing",
    ...
    fr: {
      ...
      navContact: "Contact",
      heroName: "Eneas Romero",
      heroSubtitle: "Mathématiques & Informatique",
...
// Usa 'var' para evitar error de redeclaración si el script se carga varias veces
...
function updateLangFlag() {
  ...
  // Cambia los labels de los idiomas en el menú según el idioma actual
  const langLabels = {
  ...
  document.querySelectorAll('.lang-option').forEach(option => {
    ...
    // Busca el nodo de texto después de la imagen y reemplázalo
    if (langLabels[currentLang] && langLabels[currentLang][lang]) {
      // Elimina todos los nodos de texto después de la imagen
      let foundImg = false;
      option.childNodes.forEach(node => {
        if (node === img) {
          foundImg = true;
        } else if (foundImg && node.nodeType === 3) {
          node.textContent = ' ' + langLabels[currentLang][lang];
          foundImg = false;
        }
      });
      // Si no hay nodo de texto después de la imagen, añádelo
      if (img && (!img.nextSibling || img.nextSibling.nodeType !== 3)) {
        img.after(document.createTextNode(' ' + langLabels[currentLang][lang]));
      }
    }
    // No marcar ninguna opción como activa
