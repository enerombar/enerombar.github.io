...
window.initAfterNavbar = function() {
  ...
  if (canvas) {
    ...
    function animate() {
      ...
      const opacityBoost = fullscreen ? 0.7 : 1;
      const colorBoost = fullscreen ? 1.15 : 1; // Solo un poco más claro, no blanco
      const pointOpacityBoost = fullscreen ? 0.7 : 1;
      const symbolOpacityBoost = fullscreen ? 0.7 : 1;
      ...
      baseWaveConfig.forEach(wave => {
          ...
          if (fullscreen) {
            ...
            // Mezcla con azul oscuro (rgb(10,25,40)) - 70% color original, 30% oscuro
            color = `${Math.round(rgb[0] * 0.7 + 10 * 0.3)}, ${Math.round(rgb[1] * 0.7 + 25 * 0.3)}, ${Math.round(rgb[2] * 0.7 + 40 * 0.3)}`;
          }
      ...
      points.forEach(p => {
        ...
        let pointColor = fullscreen ? 'rgba(140, 170, 220,' : 'rgba(200, 225, 255,';
      ...
      mathSymbols.forEach(s => {
        ...
        let symbolColor = fullscreen ? 'rgba(140, 170, 220,' : 'rgba(200, 225, 255,';
      ...
      const irregularColors = fullscreen
        ? [
            'rgba(20, 50, 80, OPACITY)',      // azul oscuro
            'rgba(0, 120, 140, OPACITY)',     // cian oscuro
            'rgba(40, 100, 180, OPACITY)',    // azul oscuro medio
            'rgba(70, 120, 180, OPACITY)'     // azul oscuro medio
          ]
        : [
      ...
      for (let i = 0; i < numShapes; i++) {
        ...
        let opacity = (0.025 + i * 0.01) * (fullscreen ? 0.9 : 1);
        if (fullscreen) opacity = Math.min(opacity, 0.25);
      ...
      if (fullscreen) {
        grad.addColorStop(0, 'rgba(0,140,160,0.2)');
        grad.addColorStop(0.5, 'rgba(20,50,80,0.15)');
        grad.addColorStop(1, 'rgba(40,100,180,0.12)');
      } else {
      ...
      ctx.save();
      ctx.strokeStyle = fullscreen ? `rgba(80,130,180, 0.15)` : `rgba(100, 180, 255, 0.045)`;
      ctx.lineWidth = 1.2;
      ctx.beginPath();
