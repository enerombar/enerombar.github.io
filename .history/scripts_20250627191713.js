...
window.initAfterNavbar = function() {
  ...
  if (canvas) {
    ...
    // --- NUEVO: función para obtener tamaño actual del canvas (sin fullscreen) ---
    ...
    function animate() {
      ...
      points.forEach(p => {
        ...
        ctx.fillStyle = `rgba(200, 225, 255, ${Math.min(p.opacity, 1.0)})`;
        ctx.beginPath();
      ...
      mathSymbols.forEach(s => {
        ...
        ctx.fillStyle = `rgba(200, 225, 255, ${Math.min(s.opacity, 0.85)})`;
      ...
      // Solo una onda sutil debajo del tubo, alineada
      ctx.save();
