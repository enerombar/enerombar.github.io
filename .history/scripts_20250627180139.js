...
window.initAfterNavbar = function() {
  ...
  if (hero && canvas) {
    // Helper to resize canvas to hero size in fullscreen
    resizeCanvasForFullscreen = function() {
      if (document.fullscreenElement === hero) {
        // Save original style only once
        ...
        // Position canvas absolutely over hero
        ...
        // Resize canvas element to hero size
        canvas.width = hero.offsetWidth;
        canvas.height = hero.offsetHeight;
        // Move canvas inside hero if not already
        if (canvas.parentNode !== hero) {
          hero.insertBefore(canvas, hero.firstChild);
        }
      } else if (originalCanvasStyle) {
        // Restore canvas to original style and position
        ...
        canvas.style.pointerEvents = originalCanvasStyle.pointerEvents || '';
        // Move canvas back to body if needed
