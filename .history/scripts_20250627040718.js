...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Intercepta enlaces internos para animación de salida
  document.querySelectorAll('a[href]').forEach(link => {
    // Solo enlaces internos (misma host)
    const url = new URL(link.href, window.location.origin);
    if (url.origin === window.location.origin && !link.hasAttribute('target') && !link.href.startsWith('javascript:')) {
      link.addEventListener('click', function(e) {
        // Evita animación para anclas en la misma página
