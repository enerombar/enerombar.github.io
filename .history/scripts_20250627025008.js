  ...
  if (canvas) {
    ...
    function animate() {
      ...
      for (let i = 0; i < numShapes; i++) {
        ...
        ctx.strokeStyle = `rgba(0, 234, 255, ${0.05 + i * 0.02})`; // menor opacidad
      ...
      // 5. Dibujar menos líneas onduladas horizontales y con menor opacidad
      // Tubo coloreado (gradiente) para la primera onda
      ctx.save();
      let tubeY = [];
      ctx.beginPath();
      ...
      // Segunda línea paralela para el "tubo"
      for (let x = width; x >= 0; x -= 2) {
        const y = tubeY[x / 2] + 18; // grosor del tubo
        ctx.lineTo(x, y);
      }
      ctx.closePath();
      // Gradiente para el tubo
      const grad = ctx.createLinearGradient(0, height * 0.35, 0, height * 0.35 + 18);
      ...
      // Solo una onda extra sutil debajo
      for (let l = 1; l < 2; l++) {
        ctx.save();
        ctx.strokeStyle = `rgba(0, 234, 255, 0.045)`;
        ctx.lineWidth = 1.2;
        ctx.beginPath();
        for (let x = 0; x <= width; x += 4) {
          const y = height * (0.25 + l * 0.18) +
            Math.sin(x * 0.012 + time * 0.008 + l * 1.5) * (18 + l * 6) +
            Math.cos(x * 0.02 + time * 0.005 - l * 2) * (8 + l * 3);
          if (x === 0) ctx.moveTo(x, y);
          else ctx.lineTo(x, y);
        }
        ctx.stroke();
