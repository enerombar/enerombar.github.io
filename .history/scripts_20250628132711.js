...
function updateLangFlag() {
  ...
  document.querySelectorAll('.lang-option').forEach(option => {
    ...
    if (langLabels[currentLang] && langLabels[currentLang][lang]) {
      // Cambia solo el texto, no la imagen
      // Busca el nodo de texto después de la imagen
      let textNode = Array.from(option.childNodes).find(n => n.nodeType === 3 && n.textContent.trim().length > 0);
      if (textNode) {
        textNode.textContent = ' ' + langLabels[currentLang][lang];
      } else {
        // Si no existe, añade el texto
        option.appendChild(document.createTextNode(' ' + langLabels[currentLang][lang]));
      }
    }
