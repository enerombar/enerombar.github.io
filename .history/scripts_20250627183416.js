...
window.initAfterNavbar = function() {
  ...
  if (canvas) {
    ...
    function animate() {
      ...
      const opacityBoost = fullscreen ? 3.2 : 1; // antes 2.2
      const colorBoost = fullscreen ? 1.35 : 1;  // antes 1.25
      const pointOpacityBoost = fullscreen ? 3.2 : 1; // antes 2.2
      const symbolOpacityBoost = fullscreen ? 3.2 : 1; // antes 2.2

      // 1. Draw waves
      baseWaveConfig.forEach(wave => {
          const boostedOpacity = Math.min(wave.opacity * opacityBoost, 0.9); // antes 0.7
          // Si está en fullscreen, usa un color más claro (más blanco)
          let color = wave.color;
          if (fullscreen) {
            // Mezcla el color con blanco para aclarar
            const rgb = color.split(',').map(Number);
            color = `${Math.round(rgb[0] + (255 - rgb[0]) * 0.35)}, ${Math.round(rgb[1] + (255 - rgb[1]) * 0.35)}, ${Math.round(rgb[2] + (255 - rgb[2]) * 0.35)}`; // antes 0.25
          }
      ...
      points.forEach(p => {
        ...
        // --- NUEVO: puntos más claros en fullscreen ---
        let pointColor = fullscreen ? 'rgba(255,255,255,' : 'rgba(200, 225, 255,';
        let opacity = Math.min(p.opacity * pointOpacityBoost, 1.0); // antes 0.85
      ...
      mathSymbols.forEach(s => {
        ...
        // --- NUEVO: símbolos más claros en fullscreen ---
        let symbolColor = fullscreen ? 'rgba(255,255,255,' : 'rgba(200, 225, 255,';
        let opacity = Math.min(s.opacity * symbolOpacityBoost, 0.85); // antes 0.6
      ...
      // --- NUEVO: colores más claros en fullscreen ---
      const irregularColors = fullscreen
        ? [
            ...
            'rgba(200, 240, 255, OPACITY)'     // azul muy claro
          ]
        : [
            'rgba(0, 234, 255, OPACITY)',      // azul cian
            'rgba(255,255,255, OPACITY)',      // blanco
      ...
      for (let i = 0; i < numShapes; i++) {
        ...
        // Usa color diferente para cada onda, con opacidad ajustada
        let opacity = (0.025 + i * 0.01) * (fullscreen ? 3.2 : 1); // antes 2.2
        if (fullscreen) opacity = Math.min(opacity, 0.22); // antes 0.13
      ...
      ctx.closePath();
      // --- NUEVO: gradiente más claro en fullscreen ---
      const grad = ctx.createLinearGradient(0, tubeBaseY, 0, tubeBaseY + 18);
      if (fullscreen) {
        grad.addColorStop(0, 'rgba(255,255,255,0.32)'); // antes 0.22
        grad.addColorStop(0.3, 'rgba(255,255,255,0.28)'); // antes 0.18
        grad.addColorStop(0.5, 'rgba(0,234,255,0.23)'); // antes 0.15
        grad.addColorStop(0.7, 'rgba(255,255,255,0.19)'); // antes 0.13
        grad.addColorStop(1, 'rgba(200,240,255,0.17)'); // antes 0.11
      } else {
      ...
      // Solo una onda sutil debajo del tubo, alineada (puedes dejarla azul claro)
      ctx.save();
      ctx.strokeStyle = fullscreen ? `rgba(255,255,255, 0.16)` : `rgba(100, 180, 255, 0.045)`; // antes 0.09
      ctx.lineWidth = 1.2;
      ctx.beginPath();
