...
// --- Modular background waves ---
function initBackgroundWaves() {
  if (document.querySelector('.background-waves')) return; // Ya existe
  const canvas = document.createElement('canvas');
  canvas.className = 'background-waves';
  canvas.id = 'background-waves-canvas';
  document.body.appendChild(canvas);

  // Simple animación de ondas (puedes mejorarla)
  ...
  function draw() {
    ...
    for (let x = 0; x <= width; x += 4) {
      const y = height * 0.22 + Math.sin((x / width) * 4 * Math.PI + t) * 24 + Math.cos((x / width) * 2 * Math.PI + t * 0.7) * 12;
    ...
    ctx.beginPath();
    for (let x = 0; x <= width; x += 4) {
      const y = height * 0.82 + Math.sin((x / width) * 3 * Math.PI - t * 0.7) * 32 + Math.cos((x / width) * 2 * Math.PI - t * 0.3) * 16;
      if (x === 0) ctx.moveTo(x, y);
      else ctx.lineTo(x, y);
    }
...
window.initAfterNavbar = function() {
  ...
  // Inicializa el fondo de ondas animadas en todas las páginas
  initBackgroundWaves();
};