...
window.initAfterNavbar = function() {
  ...
  if (canvas) {
    ...
    function animate() {
      ...
      const opacityBoost = fullscreen ? 2.2 : 1;
      const colorBoost = fullscreen ? 1.25 : 1;
      const pointOpacityBoost = fullscreen ? 2.2 : 1;
      const symbolOpacityBoost = fullscreen ? 2.2 : 1;

      // 1. Draw waves
      baseWaveConfig.forEach(wave => {
          const boostedOpacity = Math.min(wave.opacity * opacityBoost, 0.7);
          ...
          if (fullscreen) {
            ...
            color = `${Math.round(rgb[0] + (255 - rgb[0]) * 0.25)}, ${Math.round(rgb[1] + (255 - rgb[1]) * 0.25)}, ${Math.round(rgb[2] + (255 - rgb[2]) * 0.25)}`;
          }
      ...
      points.forEach(p => {
        ...
        let opacity = Math.min(p.opacity * pointOpacityBoost, 0.85);
      ...
      mathSymbols.forEach(s => {
        ...
        let opacity = Math.min(s.opacity * symbolOpacityBoost, 0.6);
      ...
      for (let i = 0; i < numShapes; i++) {
        ...
        let opacity = (0.025 + i * 0.01) * (fullscreen ? 2.2 : 1);
        if (fullscreen) opacity = Math.min(opacity, 0.13);
      ...
      if (fullscreen) {
        ...
        grad.addColorStop(0.5, 'rgba(0,234,255,0.15)');
        grad.addColorStop(0.7, 'rgba(255,255,255,0.13)');
        grad.addColorStop(1, 'rgba(200,240,255,0.11)');
      } else {
      ...
      ctx.save();
      ctx.strokeStyle = fullscreen ? `rgba(255,255,255, 0.09)` : `rgba(100, 180, 255, 0.045)`;
      ctx.lineWidth = 1.2;
      ctx.beginPath();
