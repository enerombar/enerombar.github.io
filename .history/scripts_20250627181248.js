...
window.initAfterNavbar = function() {
  ...
  let isHeroFullscreen = false; // <-- Añadido
  if (canvas) {
    ...
    // --- NUEVO: detectar si está en fullscreen ---
    function updateHeroFullscreenFlag() {
      isHeroFullscreen = (document.fullscreenElement === hero);
    }
    document.addEventListener('fullscreenchange', updateHeroFullscreenFlag);
    updateHeroFullscreenFlag(); // <-- Asegura valor correcto al iniciar
    ...
    function animate() {
      ...
      // --- CAMBIO: factor de opacidad para modo fullscreen ---
      const opacityFactor = isHeroFullscreen ? 1.7 : 1.0;

      // 1. Draw waves
      waveConfig.forEach(wave => {
          ctx.strokeStyle = `rgba(${wave.color}, ${(wave.opacity * opacityFactor).toFixed(3)})`;
      ...
      points.forEach(p => {
        ...
        ctx.fillStyle = `rgba(200, 225, 255, ${(p.opacity * opacityFactor).toFixed(3)})`;
        ctx.beginPath();
      ...
      mathSymbols.forEach(s => {
        ...
        ctx.fillStyle = `rgba(200, 225, 255, ${(s.opacity * opacityFactor).toFixed(3)})`;
      ...
      for (let i = 0; i < numShapes; i++) {
        ...
        let opacity = (0.025 + i * 0.01) * opacityFactor;
      ...
      ctx.closePath();
      // Gradiente con más color y blanco en el centro
      const grad = ctx.createLinearGradient(0, tubeBaseY, 0, tubeBaseY + 18);
      // --- CAMBIO: multiplica opacidad del gradiente ---
      const gradFactor = isHeroFullscreen ? 1.7 : 1.0;
      grad.addColorStop(0, `rgba(0,234,255,${(0.18*gradFactor).toFixed(3)})`);
      grad.addColorStop(0.3, `rgba(255,255,255,${(0.13*gradFactor).toFixed(3)})`);
      grad.addColorStop(0.5, `rgba(0,180,255,${(0.11*gradFactor).toFixed(3)})`);
      grad.addColorStop(0.7, `rgba(255,255,255,${(0.10*gradFactor).toFixed(3)})`);
      grad.addColorStop(1, `rgba(0,120,255,${(0.08*gradFactor).toFixed(3)})`);
      ctx.fillStyle = grad;
      ctx.fill();
      ctx.restore();
      ...
      ctx.save();
      ctx.strokeStyle = `rgba(100, 180, 255, ${(0.045 * opacityFactor).toFixed(3)})`;
      ctx.lineWidth = 1.2;
      ctx.beginPath();
