...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Mostrar navbar solo después de hacer scroll fuera del hero (y nunca volver a ocultarla)
  const navbar = document.querySelector('.navbar');
  let navbarShown = false;
  if (navbar) {
    navbar.classList.add('navbar-hidden');
    function showNavbarOnScroll() {
      // Muestra el navbar apenas el usuario haga scroll (>40px)
      if (!navbarShown && window.scrollY > 40) {
