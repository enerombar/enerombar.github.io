...
function setActiveFlag(lang) {
  const esBtn = document.querySelector('.btn-flag-es');
  const enBtn = document.querySelector('.btn-flag-en');
  if (esBtn && enBtn) {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const enBtn = document.querySelector('.btn-flag-en');
  if (esBtn && enBtn) {
    esBtn.addEventListener('click', () => switchLanguageTo('es'));
    enBtn.addEventListener('click', () => switchLanguageTo('en'));
    // Inicializa idioma y bandera activa
    setActiveFlag(currentLang);
    switchLanguageTo(currentLang);
  } else {
    // fallback para páginas sin banderas
    switchLanguageTo(currentLang);
  }
});
