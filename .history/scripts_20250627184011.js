...
window.initAfterNavbar = function() {
  ...
  if (canvas) {
    ...
    function animate() {
      ...
      const opacityBoost = fullscreen ? 3.2 : 1;
      const colorBoost = fullscreen ? 1.15 : 1; // Solo un poco más claro, no blanco
      const pointOpacityBoost = fullscreen ? 3.2 : 1;
      const symbolOpacityBoost = fullscreen ? 3.2 : 1;

      // 1. Draw waves
      baseWaveConfig.forEach(wave => {
          const boostedOpacity = Math.min(wave.opacity * opacityBoost, 0.9);
          // Si está en fullscreen, usa un azul más claro (no blanco)
          let color = wave.color;
          if (fullscreen) {
            // Mezcla el color con azul claro, no blanco
            const rgb = color.split(',').map(Number);
            // Mezcla con azul claro (rgb(120,200,255))
            color = `${Math.round(rgb[0] + (120 - rgb[0]) * 0.25)}, ${Math.round(rgb[1] + (200 - rgb[1]) * 0.25)}, ${Math.round(rgb[2] + (255 - rgb[2]) * 0.25)}`;
          }
      ...
      points.forEach(p => {
        ...
        // --- Puntos azulados claros en fullscreen ---
        let pointColor = fullscreen ? 'rgba(120,200,255,' : 'rgba(200, 225, 255,';
      ...
      mathSymbols.forEach(s => {
        ...
        // --- Símbolos azulados claros en fullscreen ---
        let symbolColor = fullscreen ? 'rgba(120,200,255,' : 'rgba(200, 225, 255,';
      ...
      // --- Colores azulados claros en fullscreen ---
      const irregularColors = fullscreen
        ? [
            'rgba(120,200,255, OPACITY)',      // azul claro
      ...
      for (let i = 0; i < numShapes; i++) {
        ...
        let opacity = (0.025 + i * 0.01) * (fullscreen ? 3.2 : 1);
      ...
      ctx.closePath();
      // --- Gradiente azul claro en fullscreen ---
      const grad = ctx.createLinearGradient(0, tubeBaseY, 0, tubeBaseY + 18);
      if (fullscreen) {
        grad.addColorStop(0, 'rgba(0,234,255,0.32)');
        grad.addColorStop(0.3, 'rgba(120,200,255,0.28)');
        grad.addColorStop(0.5, 'rgba(0,180,255,0.23)');
        grad.addColorStop(0.7, 'rgba(120,200,255,0.19)');
        grad.addColorStop(1, 'rgba(200,240,255,0.17)');
      } else {
      ...
      // Solo una onda sutil debajo del tubo, alineada (azul claro en fullscreen)
      ctx.save();
      ctx.strokeStyle = fullscreen ? `rgba(120,200,255, 0.16)` : `rgba(100, 180, 255, 0.045)`;
      ctx.lineWidth = 1.2;
      ctx.beginPath();
