...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Fondo animado: ondas + aurora + partículas luminosas + túnel fractal
  ...
  if (canvas) {
    ...
    // Partículas luminosas en espiral
    ...
    function drawParticles(time) {
      ...
      for (let i = 0; i < PARTICLES.length; i++) {
        const p = PARTICLES[i];
        // Espiral animada
        const angle = p.angle + time * p.speed + Math.sin(time * 0.0005 + i);
        const spiralRadius = p.radius + Math.sin(time * 0.001 + i) * 18;
        const x = cx + Math.cos(angle + p.phase) * spiralRadius;
        const y = cy + Math.sin(angle + p.phase) * spiralRadius;
        // Cambio de color dinámico
        const color = PARTICLE_COLORS[(p.colorIndex + Math.floor(time * 0.02 + i)) % PARTICLE_COLORS.length];
        ctx.save();
        ctx.beginPath();
        ctx.arc(x, y, p.size, 0, Math.PI * 2);
        ctx.fillStyle = color;
        ctx.globalAlpha = 0.7 + 0.2 * Math.sin(time * 0.002 + i);
